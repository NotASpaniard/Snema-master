    ...
    <main class="main-content">
        ...
        <div class="col-md-3 mb-4">
            <form action="{{ route('showtimes.store') }}" method="POST" id="showtime-form">
                ...
                <div class="mb-3">
                    ...
                    <select name="room_id" id="room_id" class="form-select" required>
                        ...
                        @foreach ($rooms as $room)
                            <option value="{{ $room->id }}">{{ $room->room_number }} - {{ $room->cinema->name }} - {{ $room->cinema->location->location_name }}</option>
                        @endforeach
                    </select>
                </div>

                <div class="mb-3">
                    <label class="form-label">Thời gian kết thúc (tự động):</label>
                    <input type="text" id="end_time_preview" class="form-control" disabled>
                </div>

                <div class="mb-3">
                    <label class="form-label">Giá cộng thêm (tự động):</label>
                    <input type="text" id="extra_price_preview" class="form-control" disabled>
                </div>
                ...
                <div class="alert alert-warning d-none" id="conflict-alert">
                    Lịch chiếu này đang bị trùng thời gian với lịch chiếu khác trong cùng phòng!
                </div>
